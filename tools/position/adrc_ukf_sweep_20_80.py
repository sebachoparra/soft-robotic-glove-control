...
# omega_c is a compile-time constant (SGC_LADRC_OMEGA_C in
# soft_glove_core_ctrl.h); this campaign runs at omega_c=0.40, matching the
# PI controller's nominal closed-loop pole, so the ADRC-vs-PI time comparison
# is bandwidth-equalized. Rebuild soft_glove_core before running if the
# header value changed. Output path is tagged accordingly to keep the
# omega_c=0.70 (as-implemented) run's data intact for comparison.
CSV_PATH = Path("/tmp/adrc_ukf_sweep_20_80_wc040.csv")


class TestNode(Node):
    def __init__(self):
        super().__init__("adrc_ukf_sweep_20_80")
...
try:
    ...
    print("Entering ADRC_UKF mode...")
    node.set_mode(ADRC_UKF_MODE)
    node.spin_for(0.30)

    if node.status.controller_mode != ADRC_UKF_MODE:
        raise RuntimeError("ADRC_UKF mode was not entered")
    ...
    print()
    print("========================================")
    print("STARTING ADRC + UKF POSITION SWEEP")
    print(SETPOINTS)
    print("========================================")
    ...
    for step_index, q_ref in enumerate(SETPOINTS, start=1):
        ...
        while rclpy.ok():
            ...
            if not node.sensor.pump_on:
                raise RuntimeError(
                    "Pump unexpectedly OFF during ADRC_UKF control"
                )
            ...
            writer.writerow([
                ...
                node.status.active_b0,
                node.status.eso_z1_pct,
                node.status.eso_z2_pct_s,
                node.status.ff_gamma,
            ...
            if now - last_print >= 0.50:
                print(
                    ...
                    f"Pf={pf:7.2f}  "
                    f"b0={node.status.active_b0:6.3f}  "
                    f"z1={node.status.eso_z1_pct:7.2f}  "
                    f"z2={node.status.eso_z2_pct_s:8.2f}  "
                    f"ff={node.status.ff_gamma:6.3f}  "
