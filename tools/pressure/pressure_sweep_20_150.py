...
SETPOINTS = [20, 40, 60, 80, 100, 120, 140, 150, 140, 120, 100, 80, 60, 40, 20]

STABLE_BAND_KPA = 3.0
STABLE_TIME_S = 5.0
MAX_STEP_TIME_S = 30.0

GLOBAL_RAW_LIMIT_KPA = 170.0
...
CSV_PATH = Path("/tmp/pressure_sweep_20_150.csv")
...
try:
    ...
    while rclpy.ok():
        ...
        if (
            node.sensor is not None
            and not node.sensor.safety_latched
            and node.sensor.watchdog_state == 0
        ):
            print("Pico latch cleared.")
            break

        if now >= ack_deadline:
            raise RuntimeError("Could not clear Pico safety latch")

    print("Entering CTRL_PRESSURE...")
    node.set_mode(3)

    mode_deadline = time.monotonic() + 2.0
    ...
    while rclpy.ok():
        ...
        if (
            ...
            and node.sensor is not None
            and node.status.controller_mode == 3
            and node.status.control_enabled
            and node.sensor.pump_on
            and not node.sensor.safety_latched
            and node.sensor.watchdog_state == 0
        ):
            print("CTRL_PRESSURE entered cleanly.")
            break

        if time.monotonic() >= mode_deadline:
            raise RuntimeError(
                "CTRL_PRESSURE entry handshake failed"
            )
    ...
    for step_index, sp in enumerate(SETPOINTS, start=1):
        ...
        while rclpy.ok():
            ...
            if now - step_start >= MAX_STEP_TIME_S:
                print(
                    f"WARNING: SET_P={sp} did not satisfy the "
                    f"stability criterion within "
                    f"{MAX_STEP_TIME_S:.0f} s. "
                    f"Final Pf={pf:.2f} kPa. "
                    f"Continuing to next setpoint."
                )
                break
